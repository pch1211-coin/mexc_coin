...
async function fetchTicker(sym) {
  ...
  // ✅ 24h High/Low 추가 (MEXC가 주는 값 그대로)
  const high24h = Number(json.data.highPrice ?? json.data.high_price ?? NaN);
  const low24h  = Number(json.data.lowPrice  ?? json.data.low_price  ?? NaN);
