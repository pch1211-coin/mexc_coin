...
async function fetchTicker(sym) {
  ...
  // ✅ 24h High/Low (필드명: high24Price / lower24Price)
  const high24 = Number(
    json.data.high24Price ??
    json.data.high24 ??
    json.data.high_price_24h ??
    json.data.highPrice ??
    NaN
  );
  const low24 = Number(
    json.data.lower24Price ??     // <- 문서 기준
    json.data.low24Price ??       // 혹시 변형
    json.data.low24 ??
    json.data.low_price_24h ??
    json.data.lowPrice ??
    NaN
  );
